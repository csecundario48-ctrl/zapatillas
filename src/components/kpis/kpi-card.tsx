    ...
    >
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-3">
          <p className="text-xs font-medium text-[#555] uppercase tracking-widest">{title}</p>
          {icon && <div className="text-[#333] mt-0.5">{icon}</div>}
        </div>
        <p className={`text-3xl font-bold tracking-tight ${text}`}>{displayValue}</p>
        {subtitle && <p className="text-xs text-[#555] mt-2">{subtitle}</p>}
      </div>
    </div>
  )
}
