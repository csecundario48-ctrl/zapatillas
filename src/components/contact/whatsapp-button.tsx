...
export function WhatsAppButton({ phone, name, withTemplates = false, label }: WhatsAppButtonProps) {
  ...
  if (!enabled) {
    return (
      <span className="inline-flex items-center gap-1.5 text-[11px] text-[#5c5c5c]" title="Sin teléfono cargado">
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-50 mt-1.5 w-52 rounded-lg border border-white/10 bg-[#15161c] shadow-xl py-1">
            <button
              type="button"
              onClick={() => open_()}
              className="w-full text-left px-3 py-2 text-[12px] text-[#cfcfcf] hover:bg-white/[0.04] transition-colors"
            >
              Abrir chat
            </button>
            ...
            {(Object.keys(waTemplates) as WaTemplateKey[]).map(key => (
              <button
                ...
                className="w-full text-left px-3 py-2 text-[12px] text-[#a8a8a8] hover:bg-white/[0.04] hover:text-white transition-colors"
              >
