    ...
    >
      ...
      <nav className="flex-1 px-2.5 py-3 space-y-4 overflow-y-auto">
        {navGroups.map(({ label, items }) => (
          <div key={label}>
            <p className="px-2.5 mb-1 text-[10px] font-semibold text-foreground/35 uppercase tracking-[0.12em]">
              {label}
            </p>
      ...
      <div className="px-5 py-3.5 border-t border-foreground/[0.06]">
        <p className="text-[10px] text-foreground/30 font-mono tracking-widest uppercase">v1.0</p>
      </div>
    </aside>
  )
}
